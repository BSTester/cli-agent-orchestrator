...
1. **Handoff (blocking)**: The message starts with `[CAO Handoff]` and includes the supervisor's terminal ID. The orchestrator automatically captures your output when you finish. Complete the task, present your deliverables, and remain online in the current terminal. Do NOT call `send_message`, and do NOT send `/exit` or `/quit` unless explicitly instructed — the orchestrator handles the return.

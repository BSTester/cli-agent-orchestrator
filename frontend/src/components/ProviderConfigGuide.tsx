...
export default function ProviderConfigGuide({
  ...
  const visibleProviders = useMemo(() => {
    ...
    return providers.filter(
      (provider: ProviderGuideProvider) => forms[provider.id]?.selected ?? provider.default_selected
    );
  }, [forms, summary?.providers]);
  ...
  const content = (
    <>
      ...
      />
      <InfoHint text="账号登录类 provider 建议先打开登录终端，再根据提示完成浏览器/设备码登录；API 类 provider 会直接写入对应配置。" />
      ...
      {loading ? (
        <SectionCard>
          <div style={{ color: "var(--text-dim)" }}>正在读取 provider 状态...</div>
        </SectionCard>
      ) : !summary ? (
        ...
        <>
          <SectionCard>
            <div style={{ display: "grid", gap: 10 }}>
              <SectionTitle title="启用范围" />
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {summary.providers.map((provider: ProviderGuideProvider) => {
                  const selected = forms[provider.id]?.selected ?? provider.default_selected;
                  return (
                    <label
                      key={provider.id}
                      style={{
                    ...
                    >
                      <input
                        ...
                        onChange={(event: ChangeEvent<HTMLInputElement>) =>
                          updateForm(provider.id, { selected: event.target.checked })
                        }
                      />
              ...
              <InfoHint text="除 OpenClaw 外，其余 provider 默认都以账号登录路径展示；Claude Code 与 Codex 可切换到 API Key 配置模式。" />
            </div>
          </SectionCard>

          {visibleProviders.map((provider: ProviderGuideProvider) => {
            ...
            const canOpenSlashLogin = Boolean(provider.login_send_command) && Boolean(activeTerminal?.terminalId);
            const isApiMode = provider.supports_api_config && form.mode === "api";
            const showAccountLogin = provider.supports_account_login && (!provider.supports_api_config || form.mode === "account");
            const shouldShowModeSwitch = provider.supports_api_config && DEFAULT_ACCOUNT_MODE_PROVIDER_IDS.has(provider.id);
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                  <div style={{ display: "grid", gap: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <StatusPill text={status.installed ? "已安装" : "未安装"} active={status.installed} />
                      <StatusPill text={status.configured ? "已配置" : "待配置"} active={status.configured} />
                      {status.detected_mode ? <StatusPill text={`当前: ${status.detected_mode}`} active /> : null}
                    </div>
                  ...
                  {shouldShowModeSwitch ? (
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                      <SecondaryButton
                        ...
                        style={{
                          padding: "6px 10px",
                          background: form.mode === "account" ? "var(--surface)" : "var(--surface2)",
                      ...
                      <SecondaryButton
                        ...
                        style={{
                          padding: "6px 10px",
                          background: form.mode === "api" ? "var(--surface)" : "var(--surface2)",
                ...
                {isApiMode ? (
                  <div style={{ display: "grid", gap: 10 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
                      ...
                      <div style={{ display: "grid", gap: 6 }}>
                        ...
                        <TextInput
                          ...
                          placeholder={provider.id === "claude_code" ? "claude-sonnet-4-6" : "gpt-5.3-codex"}
                        />
                      </div>
                    </div>
                    <div style={{ display: "grid", gap: 6 }}>
                    ...
                    {provider.id === "openclaw" ? (
                      <div style={{ display: "grid", gap: 10, marginTop: 8, paddingTop: 10, borderTop: "1px dashed var(--border)" }}>
                            ...
                            <SelectInput
                              ...
                              onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                                updateForm(provider.id, {
                                  compatibility: event.target.value as ProviderFormState["compatibility"],
                                })
                              }
                            >
                        ...
                        {form.feishuEnabled ? (
                          <div style={{ display: "grid", gap: 10, border: "1px solid var(--border)", borderRadius: 10, padding: 12, background: "var(--surface2)" }}>
                            <div style={{ color: "var(--text-bright)", fontWeight: 700 }}>飞书 / Lark 渠道</div>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
                              <div style={{ display: "grid", gap: 6 }}>
                                ...
                                <SelectInput
                                  ...
                                  onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                                    updateForm(provider.id, {
                                      feishuDomain: event.target.value as ProviderFormState["feishuDomain"],
                                    })
                                  }
                                >
                              ...
                              <div style={{ display: "grid", gap: 6 }}>
                                ...
                                <SelectInput
                                  ...
                                  onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                                    updateForm(provider.id, {
                                      feishuConnectionMode: event.target.value as ProviderFormState["feishuConnectionMode"],
                                    })
                                  }
                                >
                              ...
                              <div style={{ display: "grid", gap: 6 }}>
                                ...
                                <SelectInput
                                  ...
                                  onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                                    updateForm(provider.id, {
                                      feishuDmPolicy: event.target.value as ProviderFormState["feishuDmPolicy"],
                                    })
                                  }
                                >
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
                              ...
                              {form.feishuConnectionMode === "webhook" ? (
                                <div style={{ display: "grid", gap: 6 }}>
                                  <div style={{ color: "var(--text-dim)", fontSize: 12 }}>Verification Token</div>
                                  <TextInput
                                    value={form.feishuVerificationToken}
                                    onChange={(event: ChangeEvent<HTMLInputElement>) =>
                                      updateForm(provider.id, { feishuVerificationToken: event.target.value })
                                    }
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <PrimaryButton type="button" onClick={() => void applyProviderConfiguration(provider)} disabled={isSaving}>
                ...
                {showAccountLogin ? (
                  <div style={{ display: "grid", gap: 10 }}>
                      ...
                      text={
                        provider.id === "copilot"
                          ...
                          : provider.id === "kiro_cli"
                            ? "Kiro 在远端环境下推荐 device flow；若浏览器回调无法自动回到容器，请把完整 callback URL 粘贴到下方并由容器内发起 GET。"
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <PrimaryButton
                        type="button"
                        onClick={() => void openLoginTerminal(provider, provider.direct_login_command ? "direct" : "launch")}
                        disabled={isTerminalBusy}
                      >
                        {isTerminalBusy ? "启动中..." : (provider.direct_login_command ? "打开登录命令终端" : "打开登录终端")}
                      </PrimaryButton>
                      {provider.login_launch_command && provider.direct_login_command ? (
                        <SecondaryButton type="button" onClick={() => void openLoginTerminal(provider, "launch")}>
                          打开交互终端
                        </SecondaryButton>
                      ) : null}
                      {provider.login_send_command ? (
                        <SecondaryButton type="button" disabled={!canOpenSlashLogin} onClick={() => void sendLoginSlashCommand(provider)}>
                    ...
                    {provider.id === "kiro_cli" ? (
                      <div style={{ display: "grid", gap: 8, marginTop: 6, borderTop: "1px dashed var(--border)", paddingTop: 10 }}>
                        <div style={{ color: "var(--text-bright)", fontWeight: 700 }}>Kiro 回调补偿</div>
                        <TextInput
                          value={kiroCallbackUrl}
                          onChange={(event: ChangeEvent<HTMLInputElement>) => setKiroCallbackUrl(event.target.value)}
                          placeholder="粘贴浏览器完成登录后看到的完整 callback URL，例如 http://localhost:49153/..."
                        />
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <PrimaryButton type="button" onClick={() => void submitKiroCallback()} disabled={kiroCallbackSubmitting}>
                            {kiroCallbackSubmitting ? "提交中..." : "由容器发起 GET 回调"}
                          </PrimaryButton>
                        </div>
                      </div>
                    ) : null}
        ...
        <div style={containerStyle} onClick={() => void dismissGuide()}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
              ...
              <SecondaryButton type="button" onClick={() => void dismissGuide()} style={{ padding: "6px 10px" }}>
                关闭引导
              </SecondaryButton>
            </div>
            {content}
